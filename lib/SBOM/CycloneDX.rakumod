...
use SBOM::Annotation:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Component:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Composition:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Declarations:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Definition:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Dependency:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Formulation:ver<0.0.16>:auth<zef:lizmat>;
use SBOM::Metadata:ver<0.0.16>:auth<zef:lizmat>;
...
use SBOM::Vulnerability:ver<0.0.16>:auth<zef:lizmat>;
...
class SBOM::CycloneDX:ver<0.0.16>:auth<zef:lizmat> does SBOM {
...
#| Returns all C<SBOM::Component> objects that can be found.
    method all-components(SBOM::CycloneDX:D:) {
        ...
        with self.metadata -> $metadata {
            recursed-components( ($_,) ) with $metadata.component;
            recursed-components( ($_,) ) for  $metadata.tools;
        }
...
#| that can be found.
    method all-purls(SBOM::CycloneDX:D:) {
        self.all-components.map: { PURL.new($_) with try .purl }
    }
